...
PostBurnIn = 40000
N = BurnIn + PostBurnIn
K = 750 # lag massimo --> da inf viene troncato
...
def CSASAV(t, v_z, Z_t):
    ...
    for k in range(1, K, 1):
        TotalSum += (beta(p-1+k, q+1)/((1-b1)*beta(p,q))) * abs(Z_t[t-k])
...
def CSAAS(t, v_z, Z_t):
    ...
    TotalSum = 0
    for k in range(1, K, 1):
        TotalSum += (beta(p-1+k, q+1)/((1-b1)*beta(p,q))) * (b2m*I_m(Z_t[t-k]) + b2p*I_p(Z_t[t-k])) * abs(Z_t[t-k])
...
def CSAIG(t, v_z, Z_t):
    ...
    TotalSum = 0
    for k in range(1, K, 1):
        TotalSum += (beta(p-1+k, q+1)/((1-b1)*beta(p,q))) * Z_t[t-k]**2
...
def AS(t, v_z, Z_t):
    ...
    TotalSum = 0
    for k in range(1, K, 1):
        TotalSum += b1**(k-1) * (b2m*I_m(Z_t[t-k]) + b2p*I_p(Z_t[t-k])) * abs(Z_t[t-k])
...
def plot_model(ax, i, pdf, specif, alpha=0.025, seed=0, ylim=(-0.1, 1.1), max_lag=50):
    lags = np.arange(max_lag)

    # genera i dati
    Y, V, E = CSA_CAViaR_FZ(pdf=pdf, alpha=alpha, specif=specif, seed=seed)

    Y_acorr = sm.tsa.acf(abs(Y[-PostBurnIn:]), nlags=len(lags)-1)
    VE_acorr = sm.tsa.acf(abs(V[-PostBurnIn:]), nlags=len(lags)-1)


    # titolo automatico
    dist_name = "Normale" if pdf == "Norm" else "t-Student"
    title = f"ACF: {specif}, {dist_name}"
...
# plt.tight_layout()
plt.savefig("CAViaR_DGP_acorr.png", dpi=300, bbox_inches='tight')
plt.show()
